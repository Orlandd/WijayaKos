...
@section('container')
    ...
    <section class="container px-3 mb-6">
        <h3 class="text-3xl font-bold ">Edit Room</h3>
    </section>
    ...
    <section class="container mx-auto px-3">
        <form action="/dashboard/rooms/{{ $room->id }}" method="post" enctype="multipart/form-data">
            ...
            <input type="text" name="nama" id="nama" class="w-full rounded-lg py-2 px-3 mb-3 border-gray-200 border-2"
                value="{{ $room->name }}"><br>
            ...
            <select id="jenis" name="jenis"
                ...
                <option {{ $room->tipe == 'Standart' ? 'selected' : '' }} value="Standart">Standart</option>
                <option {{ $room->tipe == 'Premium' ? 'selected' : '' }} value="Premium">Premium</option>
                <option {{ $room->tipe == 'VIP' ? 'selected' : '' }} value="VIP">VIP</option>
            ...
            <select id="kost" name="kost"
                ...
                @foreach ($dorms as $dorm)
                    <option {{ $dorm->id == $room->dorm_id ? 'selected' : '' }} value="{{ $dorm->id }}">{{ $dorm->nama }}
                    </option>
                @endforeach
            ...
            <input type="number" min="0" name="harga" id="harga" value="{{ $room->harga }}"
                class="w-full rounded-lg py-2 px-3 mb-3 border-gray-200 border-2"><br>

            <label for="deskripsi">Description</label><br>
            <input type="text" name="deskripsi" id="deskripsi"
                class="w-full rounded-lg py-2 px-3 mb-3 border-gray-200 border-2" value="{{ $room->deskripsi }}"><br>
            <br>
            <hr>
            <br>
            <label for="facility">Facility</label><br><br>
            <button type="button" class="py-2 px-3 rounded-lg border-2 border-cyan-500" id="add">Add Facility</button>
            <button type="button" class="py-2 px-3 rounded-lg border-2 border-cyan-500" id="remove">Remove Facility</button><br><br>
                ...
                @foreach ($room->facilities as $facility)
                    <select id="facility" name="facility[]"
                        ...
                        @foreach ($facilities as $f)
                            <option {{ $facility->id == $f->id ? 'selected' : '' }} value="{{ $f->id }}">{{ $f->nama }}</option>
                        @endforeach
                    </select>
                @endforeach
            </div>
